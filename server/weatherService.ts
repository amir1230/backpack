...
export class WeatherService {
  ...
  private processForecast(forecastData: any): DailyForecast[] {
  ...
  generateTravelRecommendation(destination: string, currentWeather?: WeatherData): TravelRecommendation {
    ...
    let currentCondition: 'excellent' | 'good' | 'fair' | 'poor' = 'good';
  ...
  private findBestMonths(monthlyData: [string, any][], destination: string): string[] {
    const scored = monthlyData.map(([month, data]) => {
      let score = 0;
      
      // Temperature scoring (prefer moderate temperatures)
      if (data.avgTemp >= 18 && data.avgTemp <= 25) score += 3;
      else if (data.avgTemp >= 15 && data.avgTemp <= 28) score += 2;
      else if (data.avgTemp >= 10 && data.avgTemp <= 30) score += 1;
      
      // Rainfall scoring (prefer less rain)
      if (data.rainfall < 30) score += 3;
      else if (data.rainfall < 60) score += 2;
      else if (data.rainfall < 100) score += 1;
      
      // Sunny days scoring
      if (data.sunnyDays >= 20) score += 3;
      else if (data.sunnyDays >= 15) score += 2;
      else if (data.sunnyDays >= 10) score += 1;
  ...
  private findWorstMonths(monthlyData: [string, any][], destination: string): string[] {
    const scored = monthlyData.map(([month, data]) => {
      let score = 0;
      
      // Higher score = worse conditions
      if (data.rainfall > 120) score += 3;
      else if (data.rainfall > 80) score += 2;
      else if (data.rainfall > 50) score += 1;
      
      if (data.humidity > 85) score += 2;
      if (data.sunnyDays < 8) score += 2;
  ...
  private addDestinationSpecificAdvice(
    destination: string,
    reasons: string[],
    activities: { recommended: string[]; avoid: string[] },
    packingTips: string[],
    healthWarnings: string[]
  ) {
  ...
  private getCountryFromDestination(destination: string): string {
    const dest = destination.toLowerCase();
    if (dest.includes('lima') || dest.includes('cusco') || dest.includes('machu picchu')) return 'Peru';
    if (dest.includes('bogota') || dest.includes('cartagena') || dest.includes('medellin')) return 'Colombia';
    if (dest.includes('buenos aires') || dest.includes('mendoza') || dest.includes('bariloche')) return 'Argentina';
    if (dest.includes('rio') || dest.includes('salvador') || dest.includes('amazon')) return 'Brazil';
    if (dest.includes('santiago') || dest.includes('valparaiso') || dest.includes('atacama')) return 'Chile';
    if (dest.includes('la paz') || dest.includes('sucre') || dest.includes('uyuni')) return 'Bolivia';
  ...
  private getDefaultRecommendation(destination: string): TravelRecommendation {
    return {
      destination,
      country: this.getCountryFromDestination(destination),

...
export class GooglePlacesService {
  ...
  async nearbySearch(params: {
    ...
    try {
      const { lat, lng, radius = 5000, type, lang = 'en', pageToken, pageSize = 20 } = params;
      
      // If pageToken is provided, use it (Google requires a short delay before using nextPageToken)
      if (pageToken) {
        // Small delay for Google API requirement
        ...
        const response = await fetch(`${BASE_URL}/nearbysearch/json?${tokenParams}`);
        const data = await response.json();

        if (data.status === 'INVALID_REQUEST') {
          throw new Error('Invalid pageToken or token expired');
        }
      ...
      // Initial search without pageToken
      ...
      const response = await fetch(`${BASE_URL}/nearbysearch/json?${searchParams}`);
      const data = await response.json();

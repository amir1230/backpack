...
import { useLocalizedFormatting } from "@/hooks/useLanguageSwitch.js";
...
const createTripFormSchema = (t: any) =>
  z.object({
    destinations: z
      .array(
        z
          .object({
            ...
            startDate: z.date({
              required_error: t("trips.start_date_required"),
            }),
...
const translateFullText = async (
  ...
  const needsTranslation =
    (targetLang === "he" && !textIsHebrew) ||
    (targetLang === "en" && textIsHebrew);
...
const formatDateRange = (dateRange: string, lang: string): string => {
  ...
  if (numericMatch) {
    const [, startMonth, startDay, startYear, endMonth, endDay, endYear] =
      numericMatch;
    const startFormatted = `${startDay.padStart(2, "0")}/${startMonth.padStart(
      2,
      "0"
    )}/${startYear}`;
    const endFormatted = `${endDay.padStart(2, "0")}/${endMonth.padStart(
      2,
      "0"
    )}/${endYear}`;
  ...
  if (textMatch) {
    ...
    const startFormatted = `${startDay.padStart(
      2,
      "0"
    )}/${startMonthNum}/${year}`;
...
export default function MyTripsNew() {
  ...
  const ALL_INTERESTS = [
    ...
    { id: "luxury", icon: Sparkles, label: t("trips.luxury") },
    {
      id: "history_culture",
      icon: Camera,
      label: t("trips.interests_list.history_culture"),
    },
    {
      id: "adventure_sports",
      icon: Mountain,
      label: t("trips.interests_list.adventure_sports"),
    },
    {
      id: "nature_wildlife",
      icon: Mountain,
      label: t("trips.interests_list.nature_wildlife"),
    },
    {
      id: "food_cuisine",
      icon: Utensils,
      label: t("trips.interests_list.food_cuisine"),
    },
    {
      id: "nightlife_entertainment",
      icon: GlassWater,
      label: t("trips.interests_list.nightlife_entertainment"),
    },
    {
      id: "photography",
      icon: Camera,
      label: t("trips.interests_list.photography"),
    },
    {
      id: "architecture",
      icon: Camera,
      label: t("trips.interests_list.architecture"),
    },
    {
      id: "local_markets",
      icon: MapPin,
      label: t("trips.interests_list.local_markets"),
    },
    {
      id: "beaches_coastlines",
      icon: Waves,
      label: t("trips.interests_list.beaches_coastlines"),
    },
    {
      id: "mountains_hiking",
      icon: Mountain,
      label: t("trips.interests_list.mountains_hiking"),
    },
    {
      id: "art_museums",
      icon: Camera,
      label: t("trips.interests_list.art_museums"),
    },
    {
      id: "music_festivals",
      icon: Music,
      label: t("trips.interests_list.music_festivals"),
    },
    {
      id: "shopping",
      icon: ShoppingBag,
      label: t("trips.interests_list.shopping"),
    },
    {
      id: "wellness_relaxation",
      icon: Clock,
      label: t("trips.interests_list.wellness_relaxation"),
    },
    {
      id: "language_learning",
      icon: Globe,
      label: t("trips.interests_list.language_learning"),
    },
    {
      id: "volunteering",
      icon: Heart,
      label: t("trips.interests_list.volunteering"),
    },
  ];
  ...
  // Form state
  const [budgetSliderValue, setBudgetSliderValue] = useState([
    linearToLog(MIN_BUDGET),
  ]);
    ...
    return destinations.reduce((sum, dest) => {
      if (dest.startDate && dest.endDate) {
        const diffTime = Math.abs(
          dest.endDate.getTime() - dest.startDate.getTime()
        );
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        return sum + diffDays;
      }
  ...
  const generateAISuggestionsMutation = useMutation({
    mutationFn: async (
      data: Omit<TripFormData, "duration"> & {
        duration: number;
        startDate?: string;
        endDate?: string;
      }
      ...
      try {
        ...
        const fixedData = jsonData.map((suggestion: any) => {
          if (
            suggestion.destinationBreakdown &&
            suggestion.destinationBreakdown.length > 1
          ) {
            // Extract unique countries from destinationBreakdown
            const countries = [
              ...new Set(
                suggestion.destinationBreakdown.map((dest: any) => dest.country)
              ),
            ];
            ...
            console.log(
              ...
              suggestion.country,
              "→",
              updatedSuggestion.country
            );
            return updatedSuggestion;
          }
          console.log(
            "Not multi-city or single destination:",
  ...
  const generateItineraryMutation = useMutation({
    mutationFn: async () => {
      ...
      // Validate data before sending - check both old destination field and new destinations array
      if (
        !formData.destination &&
        (!formData.destinations ||
          formData.destinations.length === 0 ||
          !formData.destinations[0]?.country)
      ) {
        throw new Error(
          i18n.language === "he"
            ? "צור הצעות טיול תחילה"
            : "Create trip suggestions first"
        );
      }
      ...
      // Determine the destination - support both old single-destination format and new multi-city format
      ...
      if (formData.destination) {
        // Old format: single destination
        effectiveDestination =
          formData.specificCity && formData.specificCity !== "ANY"
            ? `${formData.specificCity}, ${formData.destination}`
            : formData.destination;
        effectiveStartDate = formData.startDate;
        effectiveEndDate = formData.endDate;
        effectiveDuration = calculateDurationInDays(
          formData.startDate,
          formData.endDate
        );
      } else if (formData.destinations && formData.destinations.length > 0) {
        // New format: multi-city destinations
        // Build destination string from all destinations
        effectiveDestination = formData.destinations
          .map((dest) =>
            dest.city && dest.city !== "ANY"
              ? `${dest.city}, ${dest.country}`
              : dest.country
          )
          .join(" → ");

        // Get overall start/end dates
        effectiveStartDate = getOverallStartDate();
        effectiveEndDate = getOverallEndDate();
        effectiveDuration = getTotalDays();
      } else {
      ...
      const requestData = {
        ...
        destinations: formData.destinations, // Include destinations array for multi-city support
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: t("trips.itinerary_generated_desc", {
          count: data.length,
        }),
      });
    },
  ...
  const saveItineraryMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          user_id: user.id,
          title: `${mainDestination} ${t("trips.itinerary")} - ${formatDate(
            new Date()
          )}`,
          plan_json: {
            mainDestination,
    ...
    onError: (error) => {
      ...
      toast({
        title: t("common.error"),
        description:
          error.message === t("trips.sign_in_required_save")
  ...
  const [translatedTrips, setTranslatedTrips] = useState<
    Map<number, { title: string; description: string; highlights: string[] }>
  >(new Map());
  ...
  useEffect(() => {
    const translateTrips = async () => {
      ...
      console.log(
        `[Translation] Starting translation for ${savedTrips.length} trips to ${i18n.language}`
      );
      ...
      await Promise.all(
        savedTrips.map(async (trip) => {
          try {
            ...
            console.log(
              `[Translation] Translating trip ${trip.id}: "${trip.title}"`
            );
            const translatedTitle = await translateFullText(
              trip.title,
              i18n.language
            );
            const translatedDescription = await translateFullText(
              trip.description,
              i18n.language
            );
            ...
            const translatedHighlights = await Promise.all(
              highlights.map(async (highlight) => {
                try {
                  const translated = await translateFullText(
                    highlight,
                    i18n.language
                  );
                  console.log(
                    `[Translation] Highlight: "${highlight}" → "${translated}"`
                  );
                  return translated;
                } catch (error) {
                  console.error(
                    "Failed to translate highlight:",
                    highlight,
                    error
                  );
            ...
            console.log(
              `[Translation] Completed translation for trip ${trip.id}`
            );
          } catch (error) {
  ...
  const handleGenerateAITrips = async () => {
    try {
      ...
      const destinationString = validDestinations
        .map((dest) => {
          ...
          if (dest.startDate && dest.endDate) {
            const startFormatted = format(
              dest.startDate,
              i18n.language === "he" ? "dd/MM/yyyy" : "PPP",
  ...
  const handleGenerateItinerary = async () => {
    try {
      ...
      if (
        ...
        firstSuggestion.destinationBreakdown.length > 0
      ) {
        // Multi-city trip: generate itinerary for each destination
        console.log(
          "Generating multi-city itinerary for",
  ...
  const handleGenerateItineraryForSuggestion = async (
    ...
    try {
      ...
      if (
        suggestion.destinationBreakdown &&
        suggestion.destinationBreakdown.length > 0
      ) {
        console.log(
          "Generating multi-city itinerary for",
          suggestion.destinationBreakdown.length,
          "destinations"
        );
        ...
        for (let i = 0; i < suggestion.destinationBreakdown.length; i++) {
          ...
          console.log(
            `Generating ${daysForDestination}-day itinerary for ${dest.destination}, ${dest.country}`
          );
          ...
          if (i < suggestion.destinationBreakdown.length - 1) {
            ...
            const transportationArray =
              ...
              suggestion.transportationRecommendations ||
              [];
            const transitInfo = transportationArray.find(
              (rec: any) =>
            ...
            const transitDay = {
              ...
              location:
                i18n.language === "he"
                  ? `מעבר: ${dest.destination} → ${nextDest.destination}`
                  : `Transit: ${dest.destination} → ${nextDest.destination}`,
              activities: [
                i18n.language === "he"
              ...
              transitInfo: {
                ...
                method:
                  transitInfo?.method ||
                  transitInfo?.type ||
                  (i18n.language === "he" ? "טיסה" : "Flight"),
                duration:
                  transitInfo?.duration ||
                  transitInfo?.estimatedTime ||
                  (i18n.language === "he" ? "לא צוין" : "N/A"),
                estimatedCost:
                  transitInfo?.estimatedCost ||
                  transitInfo?.cost ||
                  (i18n.language === "he" ? "לא צוין" : "N/A"),
        ...
        if (rangeMatch) {
          ...
          durationText.includes("שבוע")
        ) {
          const weeks = parseInt(durationText) || 1;
          durationDays = weeks * 7;
        } else if (
          durationText.includes("day") ||
          durationText.includes("יום")
        ) {
          durationDays = parseInt(durationText) || 7;
        }

        console.log(
          "Generating single destination itinerary for",
          suggestion.destination
        );
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : t("trips.error_generating_itinerary"),
        variant: "destructive",
      });
    } finally {
      setIsGeneratingItinerary(false);
  ...
  const { data: savedItineraries = [], isLoading: isLoadingItineraries } =
    useQuery({
      ...
      <div className="max-w-6xl mx-auto pb-20 md:pb-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 flex justify-center">
            <TabsList className="inline-flex w-auto justify-center sm:justify-evenly h-auto sm:h-10 gap-0.5 sm:gap-2 p-1">
              <TabsTrigger
                value="preferences"
              ...
              >
                <FolderOpen className="w-4 h-4 mb-0.5 sm:mb-0 flex-shrink-0" />
                <span className="text-[8px] sm:text-sm leading-[1.1] text-center w-full whitespace-normal break-words overflow-wrap-anywhere">
                  {t("trips.saved_trips")}
                </span>
              </TabsTrigger>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        >
                          <SelectValue
                            placeholder={t("trips.select_trip_type")}
                          />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="family">
                            {t("trips.trip_types.family")}
                          </SelectItem>
                          <SelectItem value="couples">
                            {t("trips.trip_types.couples")}
                          </SelectItem>
                          <SelectItem value="bachelor_party">
                            {t("trips.trip_types.bachelor_party")}
                          </SelectItem>
                          <SelectItem value="bachelorette_party">
                            {t("trips.trip_types.bachelorette_party")}
                          </SelectItem>
                          <SelectItem value="friends">
                            {t("trips.trip_types.friends")}
                          </SelectItem>
                          <SelectItem value="solo">
                            {t("trips.trip_types.solo")}
                          </SelectItem>
                          <SelectItem value="business">
                            {t("trips.trip_types.business")}
                          </SelectItem>
                          <SelectItem value="adventure">
                            {t("trips.trip_types.adventure")}
                          </SelectItem>
                          <SelectItem value="romantic">
                            {t("trips.trip_types.romantic")}
                          </SelectItem>
                          <SelectItem value="group">
                            {t("trips.trip_types.group")}
                          </SelectItem>
                          <SelectItem value="honeymoon">
                            {t("trips.trip_types.honeymoon")}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                ...
                <div className="space-y-4">
                                ...
                                <Popover>
                                  <PopoverTrigger asChild>
                                    <Button
                                      ...
                                      } justify-start flex-row-reverse text-left`}
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <Popover>
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {destination.startDate ? (
                                        format(destination.startDate, "PPP", {
                                          locale: enUS,
                                        })
                                      ) : (
                                        <span>
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <Popover>
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {destination.endDate ? (
                                        format(destination.endDate, "PPP", {
                                          locale: enUS,
                                        })
                                      ) : (
                                        <span>
                  ...
                  >
                    <p className="text-sm font-semibold text-blue-800">
                      {t("trips.total_trip_duration")}: {getTotalDays()}{" "}
                      {t("trips.days")}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="px-4">
                    ...
                    <div className="flex justify-between text-sm text-gray-600">
                      <span>
                        {i18n.language === "he"
                          ? `₪${MIN_BUDGET.toLocaleString("he-IL")}`
                          : `$${MIN_BUDGET.toLocaleString("en-US")}`}
                      </span>
                      ...
                      <span>
                        {i18n.language === "he"
                          ...
                          : `$${MAX_BUDGET.toLocaleString("en-US")}+`}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {aiSuggestions.length === 0 && !isGenerating && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-lg font-medium text-gray-700 mb-2">
                      {t("trips.no_suggestions_generated")}
                    </p>
                    ...
                    {aiSuggestions.map((suggestion, index) => {
                      ...
                      const translateTitle = () => {
                        ...
                        if (suggestion.destination.includes(" & ")) {
                          const cities = suggestion.destination
                            .split(" & ")
                            .map((city) => translateCity(city.trim()));
                          const countries = suggestion.country
                            .split(" & ")
                            .map((country) => translateCountry(country.trim()));
                        ...
                        return `${translateCity(
                          suggestion.destination
                        )}, ${translateCountry(suggestion.country)}`;
                      };
                      ...
                      return (
                        ...
                        >
                              ...
                              <div className="flex flex-wrap gap-6 w-full justify-center">
                                {suggestion.highlights.map((highlight, idx) => (
                                  ...
                                  >
                                    ...
                                    <span>
                                      {translateText(highlight, i18n.language)}
                                    </span>
                                  </div>
                                ))}
                              </div>
                              ...
                              <div className="space-y-4 border-t pt-4">
                                ...
                                >
                                  ...
                                  <h4 className="font-semibold text-gray-800">
                                    {t("trips.destination_breakdown")}
                                  </h4>
                                </div>
                                {suggestion.destinationBreakdown.map(
                                  (dest, idx) => (
                                    <div
                                      key={idx}
                                    ...
                                    >
                                      <div>
                                        <h5 className="font-bold text-purple-900">
                                          {translateCity(dest.destination)},{" "}
                                          {dest.country.includes(" & ")
                                            ? dest.country
                                                .split(" & ")
                                                .map((c) =>
                                                  translateCountry(c.trim())
                                                )
                                                .join(" & ")
                          ...
                          {suggestion.transportation &&
                            suggestion.transportation.length > 0 && (
                              <div className="space-y-4 border-t pt-4">
                                <div
                                  className={`flex items-center gap-2 ${
                                    i18n.language === "he"
                                  ...
                                  (transport, idx) => (
                                    ...
                                    >
                                      <div>
                                        ...
                                        >
                                          {translateCity(transport.from)}{" "}
                                          {i18n.language === "he" ? "←" : "→"}{" "}
                                          {translateCity(transport.to)}
                                        </h5>
                        ...
                        */}
                          ...
                          <div className="flex items-center pt-4 border-t justify-between">
                            ...
                            <Button
                              onClick={() =>
                                saveTripMutation.mutate(suggestion)
                              }
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-xl font-bold text-slate-700">
                        {t("trips.your_day_by_day_itinerary")}
                      </h3>
                    ...
                    {itinerary.map((day, dayIndex) => {
                      ...
                      const isNewDestination =
                        dayIndex === 0 ||
                        (day.destinationName &&
                          itinerary[dayIndex - 1].destinationName !==
                            day.destinationName);
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  ...
                                  <div className="bg-white p-3 rounded-lg col-span-2">
                                    ...
                                    <p className="font-semibold text-green-700">
                                      {i18n.language === "he" &&
                                      day.transitInfo.estimatedCost.includes(
                                        " - "
                                      )
                      ...
                      return (
                        <div key={stableKey}>
                          ...
                          {isNewDestination &&
                            ...
                            !day.isTransitDay && (
                              <div
                                className="bg-gradient-to-r from-purple-50 via-pink-50 to-purple-50 border-2 border-purple-200 p-5 rounded-xl mb-4 shadow-sm"
                                dir={i18n.language === "he" ? "rtl" : "ltr"}
                              >
                                <div
                                ...
                                >
                                  ...
                                  <div className="flex-1">
                                    <h4 className="text-xl font-bold text-purple-900 mb-1">
                                      {translateCity(day.destinationName)},{" "}
                                      {translateCountry(day.countryName)}
                                    </h4>
                                    {day.dateRange && (
              ...
              <CardContent>
                {!isAuthenticated ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-600">
                      {t("trips.loading_saved_itineraries")}
                    </p>
                  </div>
                            ...
                            <div className="flex flex-col gap-4">
                              ...
                              >
                                ...
                                >
                                  ...
                                  >
                                    {t("common.created")}{" "}
                                    {formatDate(new Date(itinerary.created_at))}
                                  </p>
                                </div>
                                <Button
                                  variant="ghost"
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                <div className="bg-purple-50 p-4 rounded-lg">
                                  ...
                                  >
                                    ...
                                    >
                                      {planData?.mainDestination ||
                                        t("trips.unknown_destination")}
                                    </p>
                                  </div>
                                </div>
                                      ...
                                      >
                                        ...
                                        >
                                          {t("trips.activity_days_count", {
                                            count: planData.itinerary.length,
                                          })}
                                        </span>
                                      </div>
                                    </div>
                    ...
                    {savedTrips.map((trip) => {
                      ...
                      try {
                        ...
                        if (Array.isArray(destinations) && destinations[0]) {
                          highlights = destinations[0].highlights || [];
                          bestTimeToVisit =
                            destinations[0].bestTimeToVisit || "";
                        }
                      } catch (e) {
                        // If parsing fails, highlights will remain empty
                      }
                            ...
                            <div className="flex flex-col gap-4">
                              ...
                              >
                                ...
                                >
                                  ...
                                  >
                                    {translatedTrips.get(trip.id)?.title ||
                                      trip.title}
                                  </h3>
                                  <div
                                  ...
                                  >
                                    ...
                                    >
                                      {typeof trip.destinations === "object" &&
                              ...
                              {(() => {
                                const translatedHighlights =
                                  translatedTrips.get(trip.id)?.highlights ||
                                  highlights;
                                return (
                                ...
                                <div>
                                  ...
                                  >
                                    {(() => {
                                      ...
                                      let styles: string[] = [];
                                      if (
                                        typeof trip.travelStyle === "string"
                                      ) {
                                      ...
                                      return styles.map((style, idx) => {
                                        ...
                                        return (
                                          <Badge
                                            key={idx}
                                            variant="secondary"
                                            className="bg-gradient-to-r from-orange-100 to-teal-100 text-gray-800 border-0"
                                          >
                                            {interestConfig
                                              ? interestConfig.label
                                              : translateText(
                              ...
                              >
                                {(() => {
                                  ...
                                  const existingItinerary =
                                    savedItineraries.find((itin: any) => {
                                      const planData = itin.plan_json as any;
                                      // Match by title or destination
                                      return (
                                        itin.title === trip.title ||
                                        planData?.mainDestination === trip.title
                                      );
                                    });
                                    ...
                                    return (
                                      <Button
                                        ...
                                        onClick={async () => {
                                          ...
                                          // Generate itinerary
                                          await handleGenerateItineraryForSuggestion(
                                            suggestion
                                          );
                                        }}
                                        disabled={isGeneratingItinerary}
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-center">
                {t("trips.sign_in_to_save_itinerary")}
              </DialogTitle>

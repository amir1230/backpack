...
export default function WeatherPage() {
  ...
  const availableCountries = selectedContinent
    ? getCountriesByContinent(selectedContinent).filter(
        (country: string) =>
          WORLD_DESTINATIONS[country as keyof typeof WORLD_DESTINATIONS]
      )
    : [];
  ...
  const seasonalInfo = selectedContinent
    ? getContinentSeasonalInfo(selectedContinent, t)
    : null;
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={selectedCity}
                    onValueChange={(city) =>
                      handleCitySelect(city, selectedCountry)
                    }
                  ...
                  {popularCities.map(
                    (dest: { name: string; country: string }) => (
                      <Button
                        ...
                        variant={
                          selectedCity === dest.name ? "default" : "outline"
                        }
                        className="h-auto p-3 flex flex-col items-center gap-1"
                        onClick={() =>
                          handleCitySelect(dest.name, dest.country)
                        }
                        data-testid={`quick-city-${dest.name}`}
                      >
                        <MapPin className="w-4 h-4" />
                        <span className="text-sm font-medium">
                          {getCityName(dest.name)}
                        </span>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    <SeasonIcon
                      className={`w-6 h-6 mt-1 ${seasonalInfo.color}`}
                    />
                    <div>
                      <h4 className="font-semibold text-gray-900">
                        {t("weather.best_months")}
                      </h4>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex items-start gap-3 p-4 bg-white rounded-lg">
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 mb-1">
                    {t("weather.tip_1_title")}
                  </h4>
                  <p className="text-sm text-gray-600">
                    {t("weather.tip_1_desc")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-4 bg-white rounded-lg">
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 mb-1">
                    {t("weather.tip_2_title")}
                  </h4>
                  <p className="text-sm text-gray-600">
                    {t("weather.tip_2_desc")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-4 bg-white rounded-lg">
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 mb-1">
                    {t("weather.tip_3_title")}
                  </h4>
                  <p className="text-sm text-gray-600">
                    {t("weather.tip_3_desc")}
                  </p>
                </div>
              </div>
            </div>

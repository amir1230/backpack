              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  {journey.destinations.map((dest, idx) => (
                    <div key={idx} className={`flex items-center gap-4 p-4 bg-gradient-to-r from-orange-50 to-teal-50 rounded-lg ${isRTL ? '' : ''}`}>
                      {isRTL ? (
                        <>
                          {dest.transport && (
                            <div className="flex items-center gap-2 text-sm text-gray-600 flex-row-reverse">
                              {getTransportIcon(dest.transport.type)}
                              <span dir="rtl">{translateTransportType(dest.transport.type)}</span>
                              <span dir="rtl">{formatDuration(dest.transport.duration)}</span>
                              <span dir="rtl">{formatCost(dest.transport.cost)}</span>
                            </div>
                          )}
                          <div className="flex-1 text-right">
                            <h4 className="font-bold text-lg" dir="rtl">{translateCityName(dest.name)}, {translateCountry(dest.country)}</h4>
                            <p className="text-sm text-gray-600" dir="rtl">{dest.nights} לילות</p>
                          </div>
                        ...
                        <>
                          ...
                          <div className="flex-1 text-left">
                            <h4 className="font-bold text-lg">{translateCityName(dest.name)}, {translateCountry(dest.country)}</h4>
                            <p className="text-sm text-gray-600">{dest.nights} nights</p>
                          </div>
                          {dest.transport && (
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                              {getTransportIcon(dest.transport.type)}
                              <span>{translateTransportType(dest.transport.type)}</span>
                              <span>{formatDuration(dest.transport.duration)}</span>
                              <span>{formatCost(dest.transport.cost)}</span>
                            </div>
                          )}
                        </>
                      )}

          ...
          {totalBudget > 0 && (
            <div className="space-y-3">
              <div className={`flex items-center justify-between ${i18n.language === 'he' ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm text-gray-600 font-bold">{t('budget.budget_progress')}</span>
          ...
          <CardContent>
            <div className="space-y-3">
              {(recentExpenses || []).map((expense, index) => {
                ...
                return (
                  <div key={expense.id}>
                    <div className={`flex items-baseline justify-between py-2 ${i18n.language === 'he' ? 'flex-row-reverse' : ''}`}>
                      <div className={`flex items-baseline gap-3 ${i18n.language === 'he' ? 'flex-row-reverse' : ''}`}>

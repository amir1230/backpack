...
- **AI Assistant Page**: Dedicated full-page AI assistant interface (`/ai-assistant`) that combines active chat with conversation history sidebar. Features include: auto-save of conversations 2 seconds after last message, sidebar with all past conversations, ability to view past chats or start new ones, automatic redirection from home page when user sends first message. Users can seamlessly switch between new conversations and viewing history. Uses `chat_sessions` table with JSONB message storage. Accessible from main navigation and home page sidebar.

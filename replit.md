...
- **Multi-City Trip Planning with Date Ranges**: Users can plan complex trips with multiple destinations. Each stop includes country, city (optional), start date, and end date. The system builds comprehensive itineraries like "Paris, France (Dec 1 - Dec 3) → Rome, Italy (Dec 4 - Dec 8)" with automatic total duration calculation from date ranges. Overall trip start/end dates are automatically calculated from destination dates (earliest start date and latest end date). UI features searchable country/city dropdowns with real-time filtering, date range pickers for each destination (with proper RTL ordering for Hebrew), add/remove destination buttons, and real-time trip duration summary. Form labels are positioned on the left for improved UX. Backend API accepts both multi-city format (destinations array with date ranges) and legacy single-destination format for backward compatibility.
...
- **Flight Management System**: Integrated Duffel Flights API for flight search and booking, OpenSky Network API for real-time flight tracking, and a booking management system with current and past bookings. Flight search state (origin, destination, dates, passengers, cabin class, trip type, and search results) is automatically persisted to localStorage, allowing users to navigate away and return without losing their search parameters.
